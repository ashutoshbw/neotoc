...
Some years ago, I had its idea like the aha moment and its in my dream since then. I didn't have enough skills back then. A few months ago I decided to give it a shot since I haven't found anyone working on something like this and I also gained some experience with TypeScript and React. Note that Neotoc is not tied to React, but React knowledge helped me writing the cleanup function and to be honest, I very much wanted Neotoc to use in my React projects. And I made it. This website is built with Next.js(I agree, it's kind a overkill for single page docs like this, but I did it to know it's possible).

Note that this smooth user experience also has a performance cost. It's the calculation that `requesntAnimationFrame` does at your display refresh rate. It runs on the main thread. However I haven't exeperienced any jankiness unless very long document is used like having hundreds of headings. But sometimes even though there are no perceivable jankiness, the browser console shows performance related warnings especially on startup only which I don't know yet how to eliminate.

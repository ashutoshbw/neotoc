...
export const lastModified = "2025-05-01";

# Neotoc

Most table of contents components on documentation sites simply highlight the current heading — often based on rough guesswork.
**Neotoc goes further.**
...
Neotoc is a table of contents (TOC) generator library that makes navigating within a page a joy. On mobile, tap the bottom-right button to open the TOC. On desktop, it appears by default.

It's built for documentation sites and technical blogs, but you're free to use it anywhere it feels right.

Want to tinker with Neotoc online? Check out this [CodePen](https://codepen.io/ashutoshbw/pen/Jojvvbp).

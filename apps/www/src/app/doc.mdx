...
There are many many more things that you can do. But that's for the rest of this doc. I hope you are now fully excited to try it out as I'm to share it with you.
...
- And those headings should appear in a sensible order. By sensible order, I mean:
  ...
  - There must be no gap between two heading levels when going top to bottom. That is after `<h2>`, you should not use the heading `<h4>`.

Now you have good foundation knowledge about it. Let's now dive into its API.
...
`neotoc` takes a options object. The only required option is the `io` option.

This function returns a cleaup function. If you are using `neotoc` in React or other library/framework, you will probably need to call or return this function where it is needed. For React, the pattern is like below:

```tsx
  useEffect(() => {
    return neotoc({ io: "article >> h2,h3,h4,h5,h6 >> aside" });
  }, []);
```
